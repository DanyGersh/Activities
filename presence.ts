...
async function fetchSettings(): Promise<Settings> {
  const [
    ...
    presence.getSetting<boolean>('useOriginalTitle'),
...
function getMediaTitle(mediaInfo: MediaInfo, settings: Settings): string {
  const title = settings.useOriginalTitle && mediaInfo.OriginalTitle
    ? mediaInfo.OriginalTitle
    : mediaInfo.Name
  return title || mediaInfo.Name
}

function getMediaTitleOrDefault(
  mediaInfo: MediaInfo,
  settings: Settings,
  fallback: string,
): string {
  const title = getMediaTitle(mediaInfo, settings)
...
async function buildMediaPresence(
  ...
  switch (mediaInfo.Type) {
    case 'Audio': {
      ...
      const title = getMediaTitleOrDefault(mediaInfo, settings, 'Unknown title')
      ...
      if (settings.usePresenceName)
        presenceData.name = getMediaTitle(mediaInfo, settings)
    ...
    case 'Movie': {
      ...
      const presenceData: MediaPresenceData = {
        ...
        largeImageKey: await getCoverUrl(mediaInfo, settings),
        largeImageText: `${title} (${mediaInfo.ProductionYear})`,
      }
    ...
    case 'TvChannel': {
      ...
      const title = getMediaTitleOrDefault(mediaInfo, settings, 'Live TV')
      const presenceData: MediaPresenceData = {
        type: ActivityType.Watching,
        details: title,
        largeImageKey: await getCoverUrl(mediaInfo, settings),
...
async function handleItemDetails(settings: Settings): Promise<PresenceData | null> {
  ...
  const title = getMediaTitle(data, settings)
  const presenceData: PresenceData = {
    largeImageKey: ActivityAssets.Logo,
    details: title,
  }
